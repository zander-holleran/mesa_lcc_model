...
Usage:
    python tests/performance_congestion.py                              # Run benchmark (medium collector)
    python tests/performance_congestion.py --save-baseline              # Save as baseline (label: pre)
    python tests/performance_congestion.py --verify --collector small   # Compare to pre baseline using small config
    python tests/performance_congestion.py --clean                      # Remove baseline for current label
    python tests/performance_congestion.py --runs 5                     # More runs for stable average
    python tests/performance_congestion.py --collector {small,medium,large}  # Select collector config
"""
...
def get_baseline_paths(collector_label: str):
    ...
    return (
        BASELINE_DIR / f"performance_congestion_{collector_label}_metrics.csv",
        BASELINE_DIR / f"performance_congestion_{collector_label}_trip_log.parquet",
    )
...
def get_collector_config(size: str):
    ...
    if size == "small":
        ...
        # Closest HybridCollectorConfig equivalent to the old reporting.py system:
        # - collect_every_n=100 gated collection every 100 steps in the old model
        # - batch_run=True meant only model reporters, no agent data
        # - old reporters: Step, current_toll_car, volume, at_bus_stop,
        #   bus_mode_share_recent, avg_posted_sl_delta (no tier1 equivalent), bus_user_fee (no tier1 equivalent)
        return HybridCollectorConfig(
            tier1_interval=100,
            tier1_scalars=[
        ...
        # Leaner than the old system: larger interval, minimal scalars
        return HybridCollectorConfig(
            tier1_interval=300,
            tier1_scalars=["step", "vehicle_count", "total_finished"],
            tier1_histograms=[],
...
def get_config():
    """Return the standard config for congestion performance test."""
...
def run_performance_test(num_runs: int = 3, verbose: bool = True,
    ...
    """
    Run performance benchmark under congested conditions.
    ...
    if verbose:
        print("=" * 60)
        print(f"PERFORMANCE TEST: CONGESTION (90th percentile) [{collector_label}]")
        print("=" * 60)
        print(f"Config: 3000 persons, 2 days, traffic_percentile=90, seed={SEED}")
...
def save_baseline(num_runs: int = 3, verbose: bool = True,
    ...
    if verbose:
        print("=" * 60)
        print(f"SAVING PERFORMANCE BASELINE: CONGESTION [{collector_label}]")
        print("=" * 60)
        print()
...
def verify_against_baseline(num_runs: int = 3, verbose: bool = True,
    ...
    if verbose:
        print("=" * 60)
        print(f"VERIFYING AGAINST PERFORMANCE BASELINE: CONGESTION [{collector_label}]")
        print("=" * 60)
        print()
...
Workflow for pre vs post HybridCollector comparison:
  1. On main branch:  python tests/performance_congestion.py --save-baseline
  2. On hybrid branch: python tests/performance_congestion.py --verify --collector small
                       python tests/performance_congestion.py --verify --collector medium
                       python tests/performance_congestion.py --verify --collector large
        """
    )
    ...
    parser.add_argument("--collector", choices=["small", "medium", "large", "matched", "lean"], default=None,
