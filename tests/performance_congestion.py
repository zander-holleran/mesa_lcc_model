...
Usage:
    ...
    python tests/performance_congestion.py --save-baseline    # Save as baseline
    python tests/performance_congestion.py --verify           # Compare to baseline
    python tests/performance_congestion.py --clean            # Remove baseline
...
BASELINE_METRICS = BASELINE_DIR / "performance_congestion_metrics.csv"
BASELINE_TRIP_LOG = BASELINE_DIR / "performance_congestion_trip_log.parquet"
...
def run_performance_test(num_runs: int = 3, verbose: bool = True) -> dict:
    ...
    if verbose:
        print("=" * 60)
        print("PERFORMANCE TEST: CONGESTION (90th percentile)")
        print("=" * 60)
        print(f"Config: 3000 persons, 3 days, traffic_percentile=90, seed={SEED}")
...
def save_baseline(num_runs: int = 3, verbose: bool = True) -> bool:
    ...
    if verbose:
        print("=" * 60)
        print("SAVING PERFORMANCE BASELINE: CONGESTION")
        print("=" * 60)
        print()
...
def verify_against_baseline(num_runs: int = 3, verbose: bool = True) -> bool:
    ...
    if verbose:
        print("=" * 60)
        print("VERIFYING AGAINST PERFORMANCE BASELINE: CONGESTION")
        print("=" * 60)
        print()
...
Workflow for tracking performance changes:
  1. python tests/performance_congestion.py --save-baseline
  2. Make code changes
  3. python tests/performance_congestion.py --verify
        """
    )
