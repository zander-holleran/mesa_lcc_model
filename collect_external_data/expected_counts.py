...
def get_expected_counts():
    ...
    if expected_seconds_path.exists():
        ...
        df_interp = hourly_df.reindex(second_index).interpolate(method="linear", axis=0)
        ...
        print(
            f"Transformation complete: expected_counts_seconds.csv saved to {VEHICLE_COUNTS_DIR}"
        )
